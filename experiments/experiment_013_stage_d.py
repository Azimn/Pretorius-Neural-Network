...
def sign_matched_random_edges(net, target_signs, token: bytes):
    ...
    selected = []
    for dale in (False, True):
        need = int(np.sum(target_signs == dale))
...
def recovery(scores, founder_scores, mature_scores):
    ...
    for split in ("validation", "adversarial"):
        ...
        for metric in ("js_similarity", "top1_agreement"):
            out[split][metric] = recovery_fraction(
                founder_scores[split][metric], mature_scores[split][metric], scores[split][metric]
            )
    return out
...
def run(neurons: int, training_steps: int, seeds=SEEDS):
    ...
    for donor_seed, recipient_seed in CYCLIC_PAIRS:
        donor = nets[donor_seed]
        recipient = nets[recipient_seed]
        ...
        matched_edges, match_similarity = aligned_recipient_edges(
        ...
        random_edges = sign_matched_random_edges(
            recipient["mature"], donor_signs, f"013D:{donor_seed}:{recipient_seed}:random".encode()
        )
        shuffle_rng = np.random.default_rng(
            int.from_bytes(hashlib.sha256(f"013D:{donor_seed}:{recipient_seed}:shuffle".encode()).digest()[:8], "little")
        )
        shuffled_delta = donor_delta.copy()
        # Shuffle only within Dale classes so the control cannot create a sign-class confound.
        for dale in (False, True):
        ...
        condition_scores = {
            name: evaluate_both(net, recipient["encoder"], validation, adversarial)
        ...
        pair_results.append({
            ...
            "mapped_recipient_edges_unique": int(len(set(matched_edges.tolist()))),
            "dale_sign_compatible": True,
            "mean_aligned_edge_cosine": float(np.mean(match_similarity)),
            ...
            "recovery_fraction_relative_to_virgin_to_mature": {
                name: recovery(scores, recipient["founder_scores"], recipient["mature_scores"])
                for name, scores in condition_scores.items()
            },
        })

    # Required positive control is rerun under the same code revision, not inferred from Stage B history.
    ...
    return {
        ...
        "stage": "D_preregistered_cross_topology_delta_graft",
        "issue": 15,
        "terminal_battery_touched": False,
        "raw_recurrent_edge_indices_compared_across_topologies": False,
        "seeds": list(seeds),
        "cyclic_pairs": [list(pair) for pair in CYCLIC_PAIRS],
        "neurons": neurons,
